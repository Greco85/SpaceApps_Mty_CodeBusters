...
const Analysis: React.FC = () => {
  ...
  if (enforceTemplate && selectedModel) form.append('template_model', selectedModel);
      ...
      if (!res.ok) {
        const txt = await res.text();
        throw new Error(txt || 'Batch predict failed');
      }
      const j = await res.json();
      // show first row result
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-space-dark/50 backdrop-blur-sm border border-space-blue/30 rounded-lg p-8 mb-8">
        ...
        <div className="mt-4 flex items-center space-x-3">
          ...
          <button onClick={fetchTemplate} className="px-3 py-2 bg-blue-600 text-white rounded">Mostrar plantilla CSV</button>
          <button onClick={analyzeAll} className="px-3 py-2 bg-purple-600 text-white rounded">Analizar todo (modelo seleccionado)</button>
        </div>
      ...
      {analysisResult && (
        <div className="bg-space-dark/50 backdrop-blur-sm border border-space-blue/30 rounded-lg p-8">
          ...
          {templatePreview && (
            <div className="mt-6">
              <h4 className="text-md font-medium mb-2">Plantilla CSV (preview)</h4>
              <div className="overflow-auto max-h-40 bg-gray-900 p-2 rounded">
                <table className="w-full text-xs table-auto">
                  <tbody>
                    {templatePreview.map((r, i) => (
                      <tr key={i} className="border-b border-gray-800">
                        <td className="px-2 py-1">{String(Object.values(r)[0])}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
